...
def main():
    ...
    while simulation_app.is_running():
        ...
        #cube_position=np.array([0.5, 0, 0.5]),
        #cube_orientation=np.array([0, 1, 0, 0]),
        ...
        if (
            ...
            ee_translation_goal = cube_position+ [0, -0.001, 0.045]
        ...
        if cmd_plan is not None:    # 有规划则执行规划，夹爪打开
            ...
            flag = True    # 无规划，flag切为true，可以进行移动操作
            my_denso.gripper.apply_action(
        ...
        if (np.max(np.abs(sim_js.velocities)) < 0.1) :
            robot_static = True
        ...
        if (flag and robot_static):
            ...
            attach_obj = motion_gen.attach_objects_to_robot(
                cu_js1,
                [target],
                sphere_fit_type=SphereFitType.VOXEL_VOLUME_SAMPLE_SURFACE,
                world_objects_pose_offset=Pose.from_list([0, 0, 0.005, 1, 0, 0, 0], tensor_args),
            )
            ...
            if (
                np.linalg.norm(place_position - des_pose) > 1e-3
                and robot_static
            ):  
                ee_translation_goal1 = place_position+ [0, -0.001, 0]
                ...
                if succ:
                    ...
                    cmd_plan1 = cmd_plan1.get_ordered_joint_state(common_js_names)
                    cmd_idx1 = 0
                else:
                    carb.log_warn("Plan did not converge to a solution.  No action is being taken.")
