    ...
    def __init__(
        ...
        if not prim.IsValid():
            ...
            if self._end_effector_prim_name is None:
                self._end_effector_prim_path = prim_path + "/base_hand"
            else:
                self._end_effector_prim_path = prim_path + "/" + end_effector_prim_name
            if gripper_dof_names is None:
                gripper_dof_names = ["left_outer_knuckle_joint", "right_outer_knuckle_joint"]
            if gripper_open_position is None:
                gripper_open_position = np.array([0.01, 0.01]) / get_stage_units()
...
class CuroboController(BaseController):
    def __init__(
        ...
        self.robot_cfg = load_yaml(join_path(get_robot_configs_path(), "flexiv_grav.yml"))["robot_cfg"]
    ...
    def reached_target(self, observations: dict) -> bool:
        curr_ee_position = observations["rizon4"]["end_effector_position"]
        if np.linalg.norm(
            self.my_task.target_position - curr_ee_position
        ) < 0.004 and (  # This is half gripper width, curobo succ threshold is 0.5 cm
...
class MultiModalStacking(BaseStacking):
    ...
    def get_place_position(self, observations: dict) -> None:
        ...
        ee_to_grasped_cube = (
            observations["rizon4"]["end_effector_position"][2]
    ...
    def set_robot(self) -> Flexiv:
        flexiv_prim_path = find_unique_string_name(
            initial_name="/World/rizon4", is_unique_fn=lambda x: not is_prim_path_valid(x)
        )
        flexiv_robot_name = find_unique_string_name(
            initial_name="rizon4", is_unique_fn=lambda x: not self.scene.object_exists(x)
        )
        return Flexiv(
            prim_path=flexiv_prim_path, name=flexiv_robot_name, end_effector_prim_name="base_hand"
        )


robot_prim_path = "/World/rizon4/base_link"
ignore_substring = ["Flexiv", "TargetCube", "material", "Plane"]
...
rizon4 = my_world.scene.get_object(robot_name)
my_controller = CuroboController(my_world=my_world, my_task=my_task)
articulation_controller = rizon4.get_articulation_controller()
set_camera_view(eye=[2, 0, 1], target=[0.00, 0.00, 0.00], camera_prim_path="/OmniverseKit_Persp")
wait_steps = 8

rizon4.set_solver_velocity_iteration_count(4)
rizon4.set_solver_position_iteration_count(124)
...
# exit()
rizon4.gripper.open()
print(rizon4.gripper.get_joint_positions()[0])
...
while simulation_app.is_running():
    ...
    observations = my_world.get_observations()
    sim_js = rizon4.get_joints_state()

    if my_controller.reached_target(observations):
        if rizon4.gripper.get_joint_positions()[0] < 0.0035:  # reached placing target
            rizon4.gripper.open()
            for _ in range(wait_steps):
                my_world.step(render=True)
