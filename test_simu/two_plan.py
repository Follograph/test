...
def main():
    ...
    while simulation_app.is_running():
        ...
        place_position, place_orientation = target.get_world_pose()
        if des_pose is None:
        ...
        if flag:
            ...
            for _ in range(2):
                my_world.step(render=False)
            if k <= 1:
                idx_list1 = [robot.get_dof_index(x) for x in j_names]
                robot.set_joint_positions(default_config, idx_list1)
            ...
            robot_static = False
            if (np.max(np.abs(sim_js1.velocities)) < 0.2) :
                robot_static = True
            # print(robot_static)
            ...
            if (
                ...
                ee_translation_goal1 = place_position+ [0, -0.001, 0.045]
